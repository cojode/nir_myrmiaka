...
class WorkManagementService:
    ...
    async def verify_exists_and_role_specified(self, user_id, role: str):
        user: UsersUserprofile = await self.users_userprofile_repo.find_one(user_id=user_id)
        if user == None:
    ...
    async def create_assignment(self, payload: AssignmentCreateRequest):
        new_assignment_data = self._extract_from_payload(payload,
            'student_id',
            'teacher_id',
            'text'
        )
        self.verify_exists_and_role_specified(new_assignment_data['student_id'], 'Student')
        self.verify_exists_and_role_specified(new_assignment_data['teacher_id'], 'Teacher')
    ...
    async def accept_assignment(self, teacher_id, semestr, assignment_id):
        self.verify_exists_and_role_specified(teacher_id, 'Teacher')
        target_base_assignment: BaseAssignment = await self.base_assignment_repo.find_by_id(assignment_id)
